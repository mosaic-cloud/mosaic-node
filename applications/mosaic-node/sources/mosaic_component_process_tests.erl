...
configure_1 (Type, Identifier, defaults, ExtraOptions)
		when (Type =:= 'mosaic-components:rabbitmq'), is_list (ExtraOptions) ->
	try
		Executable = enforce_ok_1 (mosaic_generic_coders:os_bin_get (<<"mosaic-components-rabbitmq--run-component">>)),
		Options = [
				{harness, [
					{argument0, <<"[", (erlang:atom_to_binary (Type, utf8)) / binary, "#", (enforce_ok_1 (mosaic_component_coders:encode_component (Identifier))) / binary, "]">>}]},
				{execute, [
...
configure_1 (Type, Identifier, defaults, ExtraOptions)
		when (Type =:= 'mosaic-components:riak-kv'), is_list (ExtraOptions) ->
	try
		Executable = enforce_ok_1 (mosaic_generic_coders:os_bin_get (<<"mosaic-components-riak-kv--run-component">>)),
		Options = [
				{harness, [
					{argument0, <<"[", (erlang:atom_to_binary (Type, utf8)) / binary, "#", (enforce_ok_1 (mosaic_component_coders:encode_component (Identifier))) / binary, "]">>}]},
				{execute, [
...
configure_1 (Type, Identifier, {json, [Class, Configuration, ClassPath]}, ExtraOptions)
		when (Type =:= 'mosaic-components:java-component-container'), is_binary (Class), is_list (ClassPath), is_list (ExtraOptions) ->
	try
...
configure_1 (Type, Identifier, {json, [Class, Configuration, ClassPath, Logger]}, ExtraOptions)
		when (Type =:= 'mosaic-components:java-component-container'), is_binary (Class), is_list (ClassPath), is_binary (Logger), is_list (ExtraOptions) ->
	try
		Executable = enforce_ok_1 (mosaic_generic_coders:os_bin_get (<<"mosaic-components-java-component-container--run-component">>)),
		Options = [
				{harness, [
...
configure_1 (Type, Identifier, {json, [Configuration, ClassPath]}, ExtraOptions)
		when (Type =:= 'mosaic-components:java-cloudlet-container'), is_list (ClassPath), is_list (ExtraOptions) ->
	try
...
configure_1 (Type, Identifier, {json, [Configuration]}, ExtraOptions)
	...
	try
		...
		Executable = enforce_ok_1 (mosaic_generic_coders:os_bin_get (<<"mosaic-components-java-driver-", TypeSuffix / binary, "--run-component">>)),
		Options = [
				{harness, [
					{argument0, <<"[", (erlang:atom_to_binary (Type, utf8)) / binary, "#", (enforce_ok_1 (mosaic_component_coders:encode_component (Identifier))) / binary, "]">>}]},
				{execute, [
...
configure_1 (Type, Identifier, {json, [Configuration]}, ExtraOptions)
		when (Type =:= 'mosaic-examples-realtime-feeds:frontend-java'), is_list (ExtraOptions) ->
	try
...
configure_1 (Type, Identifier, {json, [Configuration]}, ExtraOptions)
		when (Type =:= 'mosaic-examples-realtime-feeds:indexer-java'), is_list (ExtraOptions) ->
	try
...
configure_1 (Type, Identifier, {json, JarPath}, ExtraOptions)
		when (Type =:= 'mosaic-tests:java-component'), is_binary (JarPath), is_list (ExtraOptions) ->
	try
		Options = [
				{harness, [
					{argument0, <<"[", (erlang:atom_to_binary (Type, utf8)) / binary, "#", (enforce_ok_1 (mosaic_component_coders:encode_component (Identifier))) / binary, "]">>}]},
...
configure_1 (Type, Identifier, {json, [MainClass, ClassPath]}, ExtraOptions)
		when (Type =:= 'mosaic-tests:java-component'), is_binary (MainClass), is_binary (ClassPath), is_list (ExtraOptions) ->
	try
		Options = [
				{harness, [
					{argument0, <<"[", (erlang:atom_to_binary (Type, utf8)) / binary, "#", (enforce_ok_1 (mosaic_component_coders:encode_component (Identifier))) / binary, "]">>}]},
...
configure_1 (Type, Identifier, {json, [Token, Endpoint]}, ExtraOptions)
			when (Type =:= 'mosaic-tests:socat'), is_binary (Token), is_binary (Endpoint), is_list (ExtraOptions) ->
	try
...
configure_1 (Type, Identifier, {json, [Executable, Arguments]}, ExtraOptions)
			when (Type =:= 'mosaic-tests:exec'), is_list (ExtraOptions) ->
	try
		Options = [
				{harness, [
					{argument0, <<"[", (erlang:atom_to_binary (Type, utf8)) / binary, "#", (enforce_ok_1 (mosaic_component_coders:encode_component (Identifier))) / binary, "]">>}]},
