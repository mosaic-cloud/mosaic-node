...
options (defaults) ->
	case application:get_env (mosaic_node, webmachine_address) of
		{ok, {Address, Port}} when is_list (Address), is_integer (Port), (Port >= 0), (Port < 65536) ->
			case dispatches ([mosaic_cluster_tools_wm_v1, mosaic_cluster_processes_wm_v1, mosaic_static_resources_wm]) of
				{ok, Dispatches} ->
					Options = [
							{ip, Address},
							{port, Port},
