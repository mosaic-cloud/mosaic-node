...
test_start_stop ({defaults}) ->
	...
	{ok, Configuration} = mosaic_harness_coders:decode_frontend_configuration (term, [{controller, Self}, {controller_token, SelfToken}]),
...
test_execute ({sleep, wait}) ->
	...
	SelfToken = erlang:make_ref (),
	{ok, Configuration} = mosaic_harness_coders:decode_frontend_configuration (term, [{controller, Self}, {controller_token, SelfToken}]),
	{ok, ExecuteSpecification} = mosaic_harness_coders:decode_frontend_execute_specification (term, [{executable, <<"/bin/sleep">>}, {arguments, [<<"0.1s">>]}]),
	{ok, Harness} = mosaic_harness_frontend:start_link (Configuration),
...
test_execute ({sleep, nowait}) ->
	...
	SelfToken = erlang:make_ref (),
	{ok, Configuration} = mosaic_harness_coders:decode_frontend_configuration (term, [{controller, Self}, {controller_token, SelfToken}]),
	{ok, ExecuteSpecification} = mosaic_harness_coders:decode_frontend_execute_specification (term, [{executable, <<"/bin/sleep">>}, {arguments, [<<"1h">>]}]),
	{ok, Harness} = mosaic_harness_frontend:start_link (Configuration),
	ok = mosaic_harness_frontend:execute (Harness, ExecuteSpecification),
	ok = mosaic_harness_frontend:stop (Harness),
	ok = mosaic_tests:wait (Harness),
...
test_execute ({cat, nowait}) ->
	...
	SelfToken = erlang:make_ref (),
	{ok, Configuration} = mosaic_harness_coders:decode_frontend_configuration (term, [{controller, Self}, {controller_token, SelfToken}]),
	{ok, ExecuteSpecification} = mosaic_harness_coders:decode_frontend_execute_specification (term, [{executable, <<"/bin/cat">>}, {arguments, []}]),
	{ok, Harness} = mosaic_harness_frontend:start_link (Configuration),
	ok = mosaic_harness_frontend:execute (Harness, ExecuteSpecification),
	ok = mosaic_harness_frontend:stop (Harness),
	ok = mosaic_tests:wait (Harness),
...
test_signal ({defaults}) ->
	...
	{ok, Configuration} = mosaic_harness_coders:decode_frontend_configuration (term, [{controller, Self}, {controller_token, SelfToken}]),
	{ok, ExecuteSpecification} = mosaic_harness_coders:decode_frontend_execute_specification (term, [{executable, <<"/bin/sleep">>}, {arguments, [<<"1h">>]}]),
	{ok, SignalSpecification} = mosaic_harness_coders:decode_frontend_signal_specification (term, terminate),
	{ok, Harness} = mosaic_harness_frontend:start_link (Configuration),
	ok = mosaic_harness_frontend:execute (Harness, ExecuteSpecification),
...
test_push_packet ({defaults}) ->
	...
	SelfToken = erlang:make_ref (),
	{ok, Configuration} = mosaic_harness_coders:decode_frontend_configuration (term, [{controller, Self}, {controller_token, SelfToken}]),
	{ok, ExecuteSpecification} = mosaic_harness_coders:decode_frontend_execute_specification (term, [{executable, <<"/bin/cat">>}]),
	{ok, Harness} = mosaic_harness_frontend:start_link (Configuration),
	ok = mosaic_harness_frontend:execute (Harness, ExecuteSpecification),
